...
```{r}
#Use the function hist with an appropriate number of breaks to visualize when interactions tend to take place
```
...
non.event.ids<-apply(cbind(tms.shift,event.ids),1, function(x,tau){resample(setdiff((1:length(tau))[(x[1]>tau)&(x[1]<(tau+T))],c(NA,x[2])))[1]},tau=tau)
```
