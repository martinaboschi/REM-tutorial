...
$$ x_{r}^{\text{in-degree}}(t) = \sum_{ s \in \mathcal{S} } \sum_{t_i < t} {1} \{ s_i = s, r_i=r \}  $$
...
We will start with the baseline model (M) and will continue adding more refined network statistics. More specifically,}

- \emph{M includes statistics computed over the whole observation period;}
- \emph{M\_SLT includes statistics computed over short and long time windows;}
...
Nodal endogenous attributes like _RecNetBal_ are also included. This statistics refer to the difference between the lent and the borrowed amount of a bank. The statistics _EdBalDif_ is similarly defined. It refers to the same difference but evaluated across dyads, i.e. across the pairs of lenders (senders) and borrowers (receivers).

Please note that nodal statistics are referred to receivers (Rec...) rather than senders. This is because the NCCS is sender-based to account for lenders' heterogeneity.}
...
#### 4.3.2. Short/Long Term Network Statistics

\colorbox{darkseagreen}{Demo 5.4}:  Incorporate in the model \textcolor{mauvetaupe}{short-term network statistics}.

\emph{Computing network statistics over both short and long temporal frames is essential for gaining a comprehensive understanding of network behavior and dynamics.}

-   __Short temporal frames__:
-   \emph{Capturing temporary behavior: Short frames allow you to capture rapid fluctuations and short-lived events in the network.}
-   __Long Temporal Frames__:
-   _Identifying trends and seasonality_: \emph{Long frames reveal longer-term patterns and trends in network traffic, such as:}
-   _Long-term growth or decline_: \emph{Understanding overall trends in network usage.}

\emph{In the context of the interbank market, analyzing short-term statistics can reveal how banks react to immediate events (e.g., changes in interest rates, economic news).  Long-term statistics might show how relationships between banks evolve over time, or how the market responds to broader economic cycles.}

\emph{To understand how banks interact in the short term, we focus on specific time periods, or windows. We have chosen a window of 45 days for this analysis. Why 45 days? This is roughly the average amount of time a bank keeps funds borrowed from other banks before needing to repay or adjust them.}
...
#### 4.3.3. Time-Weighted Network Statistics

\colorbox{darkseagreen}{Demo 5.5}:  Incorporate in the model \textcolor{mauvetaupe}{time-weighted network statistics}.
...
#### 4.3.4. Event-Weighted Network Statistics

\colorbox{pastelpurple}{Supplementary 2}: Inspect \textcolor{mauvetaupe}{event-weighted network statistics} and further model formulations. 

\emph{The relationships between nodes are often represented as simple connections without considering the varying strength or importance of these interactions. Event-weighted network statistics address this limitation by incorporating the magnitude or frequency of events into the analysis.}

\emph{In the context of interbank transactions, the event weight is typically the amount transacted. This means that larger transactions contribute more to the network statistics than smaller ones, providing a more nuanced view of the interbank market.}

##### Why event-weighted statistics matter

\emph{By considering transaction amounts, event-weighted statistics provide a richer understanding of the interbank market. They allow us to identify:}

-   __Key players__:\emph{ Banks with large transaction volumes or significant mutual exchange relationships.}
-   __Dominant flows__: \emph{The direction and magnitude of net fund flows between banks.}

\emph{For our interbank network, we focus on three types of dyadic (pairwise) event-weighted statistics, all computed considering the amounts of repeated and reciprocated transactions.}

-   __Repeated Transaction Amount__: \emph{The total amount transacted between two banks over a given period. This captures the overall volume of business between the banks.}

-   __Reciprocated Transaction Amount__: \emph{The total amount exchanged in both directions between two banks over a given period. This measures the degree of mutual exchange and interdependence.}

##### Event-weighted reciprocity statistics

\emph{Let us consider reciprocity as an illustrative case. The same scheme is applied to repeated transactions (\_Trt\_ as a short form for trust)}

-   __WeightReciprocityEdgeStatistic (EdWeiRec)__: \emph{This statistic calculates the total amount of money a borrower has sent back to a lender in all their past interactions. It provides a measure of the borrower's overall repayment obligation fulfilled towards the lender.}
-   __NormalizedWeightReciprocityEdgeStatistic (EdNORWeiRec)__: \emph{This statistic builds upon EdWeiRec by incorporating normalization. It calculates the average amount of money a borrower has sent back per reciprocal interaction in the past, providing a more nuanced view of reciprocity compared to just the total sum.}
-   __EventWeightReciprocityEdgeStatistic (EdRecEvtWei)__ \emph{This statistic considers both the frequency of reciprocal transactions and the weight of the current transaction, i.e., its amount}
-   __EventWeightNormalizedWeightReciprocityEdgeStatistic (EdNORWeiRecEvtWei)__: \emph{This statistic considers both the frequency of past interactions (reflected in the average amount) and the significance of the current transaction relative to the average size of past reciprocal transactions.}

###### M_EW1

\emph{Beyond regular network statistics for repetition and reciprocity, this model specification includes EdWeiTrt and EdWeiRec statistics.}

\emph{Let us focus on reciprocity as an illustrative case. While} _EdRec_ \emph{calculates the raw count of transaction events initiated by the borrower and directed towards the lender,} _EdWeiRec_ \emph{Calculates the total amount of money involved in transactions initiated by the borrower and directed towards the lender.}

\emph{This statistic goes beyond the raw count and considers the total monetary value flowing in the reciprocal direction. A high value suggests a significant financial exchange from the borrower to the lender.}
...
\emph{This model specification includes EdNORWeiTrt and EdNORWeiRec statistics.}

_EdNORWeiRec_ \emph{calculates the average amount of money exchanged per transaction initiated by the borrower and directed towards the lender.}

\emph{This statistic provides a more nuanced view of the reciprocity by considering the average transaction size. A high value suggests frequent interactions with a significant average amount flowing from the borrower to the lender.}
...
\emph{This model specification includes EdTrtEvtWei and EdRecEvtWei statistics.}

\emph{EdRecEvtWei calculates a combined weight for reciprocity by multiplying:}

-   \emph{the number of transactions initiated by the borrower and directed towards the lender and}
-   \emph{the amount of the current transaction}

\emph{This statistic considers both the frequency of reciprocal interactions (number of transactions) and the significance of the current transaction (its amount). A high value suggests frequent reciprocal interactions with a high-value current transaction.}
